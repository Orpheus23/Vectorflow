...
struct tensor
    {
        ...
        __device__ __host__ T operator() (const int j , const int dims)
        {
            ...
            {
                int idxs = (j/ ((*(stride+i))/(*(shape+0))));
                result += (*(stride+i))*(idxs*(i==0)+idxs%(*(shape+i))*(i!=0));
...
{   
    ...
    public:
        ...
        __host__ void to_gpu()
        {
            ...
            cudaMemcpy(mat.shape, dimension_list.data(), sizeof(T)*stride_vector.size(), cudaMemcpyHostToDevice);
